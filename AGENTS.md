...
- **`core/autostart.py` escreve no registro real do Windows** (`HKCU\Software\Microsoft\Windows\CurrentVersion\Run`). `sync()` é idempotente (só escreve/remove se o estado atual divergir) — prefira sempre `sync()` a chamar `enable()`/`disable()` diretamente. Em testes, sempre faça monkeypatch de `autostart.winreg.*`/`autostart.enable`/`autostart.disable` — nunca deixe um teste tocar o registro de verdade.

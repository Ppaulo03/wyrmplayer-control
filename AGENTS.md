...
- **Spicetify se recusa a rodar enquanto elevado** (por padrão), e `src/main.py` roda o WyrmPlayerControl inteiro como admin — logo qualquer subprocess `spicetify` herda essa elevação e falha. `configure_extension`/`apply_changes` aceitam `bypass_admin: bool` (adiciona `--bypass-admin`), mas isso só deve ser passado `True` depois de consentimento explícito do usuário (`win32.confirm_dialog` no fluxo da tray) — o auto-check no startup (`run_startup_check`) nunca bypassa sozinho, só avisa e sugere rodar manualmente sem admin.
